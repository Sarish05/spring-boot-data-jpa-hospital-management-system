...
public class AuthUtil {
    ...
    public String determineProviderFromOAuth2User(OAuth2User oAuth2User, String registrationId){
    ...
    public String determineUsernameFromOAuth2User(OAuth2User oAuth2User, String registrationId, String providerId) {
        //Extract email if oAuth2User has
        String email = oAuth2User.getAttribute("email");
        if(email != null && !email.isBlank()){
            return email;
        }
        //not neccessarily each provider may have email field, but username cant be empty, we need to have it
        return switch(registrationId.toLowerCase()){
            case "google" -> oAuth2User.getAttribute("sub");
